...
function ensureContextMenu() {
  try {
    browser.menus.create({
      id: "mw-define-word",
      title: "Define with Chrome Quick Dictionary",
      contexts: ["page", "selection"]
    }, () => {
      // Ignore duplicate-ID errors when Chrome restores a persisted menu.
...
// Register once at service-worker startup as well. Duplicate-ID errors are harmless.
ensureContextMenu();
...
browser.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  lookupWord(message.word).then(sendResponse).catch((error) => {
    console.error(error);
    sendResponse({ ok: false, error: "The dictionary lookup failed." });
  });
  return true;
});
